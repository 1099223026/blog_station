...
CREATE TABLE `tab_posts` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `post_author` varchar(20) DEFAULT '0' COMMENT '发表者id',
...
) ENGINE=MyISAM AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='Portal 文章分类对应表';
...
-- ----------------------------
INSERT INTO `tab_users` VALUES ('1', 'admin', '###1d156556d37b034eb8752155285e92b6', 'admin', '1099223026@qq.com', '', null, '0', '2000-01-01', null, '202.202.218.214', '2017-04-18 15:47:30', '2017-03-14 05:51:31', '', '1', '0', '1', '0', '');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v_article_mesh` AS select `v_article_term`.`id` AS `id`,group_concat(`v_article_term`.`name` separator ',') AS `name`,`v_article_term`.`post_keywords` AS `post_keywords`,`v_article_term`.`post_source` AS `post_source`,`v_article_term`.`post_date` AS `post_date`,`v_article_term`.`post_content` AS `post_content`,`v_article_term`.`post_excerpt` AS `post_excerpt`,`v_article_term`.`post_title` AS `post_title`,`v_article_term`.`recommended` AS `recommended`,`v_article_term`.`smeta` AS `smeta`,`v_article_term`.`author` AS `author` from `v_article_term` group by `v_article_term`.`id` ;

-- ----------------------------
-- View structure for v_article_term
-- ----------------------------
DROP VIEW IF EXISTS `v_article_term`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v_article_term` AS select `tab_posts`.`id` AS `id`,`tab_posts`.`post_author` AS `author`,`tab_posts`.`post_keywords` AS `post_keywords`,`tab_posts`.`post_source` AS `post_source`,`tab_posts`.`post_date` AS `post_date`,`tab_posts`.`post_content` AS `post_content`,`tab_posts`.`post_excerpt` AS `post_excerpt`,`tab_posts`.`smeta` AS `smeta`,`tab_posts`.`post_title` AS `post_title`,`tab_posts`.`recommended` AS `recommended`,`tab_terms`.`name` AS `name` from ((`tab_terms` join `tab_posts`) join `tab_term_relationships`) where ((`tab_posts`.`id` = `tab_term_relationships`.`object_id`) and (`tab_terms`.`term_id` = `tab_term_relationships`.`term_id`)) ;

-- ----------------------------
-- Procedure structure for p_replace_article_author
-- ----------------------------
DROP PROCEDURE IF EXISTS `p_replace_article_author`;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `p_replace_article_author`()
BEGIN
# 存储过程说明：对tab_posts中的author字段进行修改，为作者的名字,如果有已经修改过的记录，不修改。
# 注：游标必须放在所有变量定义之后
# 保存id对应的用户名
...
# 定义游标
DECLARE cur CURSOR FOR SELECT post_author FROM tab_posts;
# 将结束标志绑定到游标
...
read_loop: LOOP
	# 提取游标数据
	FETCH cur INTO authorId;
	# 声明结束
	IF done THEN
		LEAVE read_loop;
	END IF;
	# 做点什么
	# 判断一下是否为数字字符串 是则返回0，反之1
	# 取反ABS(status-1)
	IF ABS((authorId  REGEXP '[^0-9]')-1) THEN
		# 根据id找到tab_users中的用户名 并赋值
		SELECT user_login INTO authorName FROM tab_users WHERE id = authorId;
		# 更新视图中的作者
		# 之前一直更新不上，值一直为0或者是null，原因是字段类型不匹配，把tab_posts中的int修改为varchar
		UPDATE v_article_term SET `author` = authorName where `author` = authorId;
	END IF;
END LOOP;
# 关闭游标
CLOSE cur;
...
BEGIN
# 存储过程说明：相当于一个main函数，组合各个存储过程进行工作
# 定义视图是否存在变量
...
# 调用来完成对tab_posts中的author字段进行修改，为作者的名字
CALL p_replace_article_author;
...
IF is_exist THEN
	...
	post_source,post_date, post_content, post_excerpt, post_title,recommended,smeta,author 
...
ELSE
	...
	post_date, post_content, post_excerpt, post_title,recommended,smeta,author 
	FROM v_article_term GROUP BY id;
END IF;
END
;;

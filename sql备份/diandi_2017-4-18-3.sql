...
CREATE TABLE `tab_posts` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `post_author` bigint(20) DEFAULT '0' COMMENT '发表者id',
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v_article_mesh` AS select `v_article_term`.`id` AS `id`,group_concat(`v_article_term`.`name` separator ',') AS `name`,`v_article_term`.`post_keywords` AS `post_keywords`,`v_article_term`.`post_source` AS `post_source`,`v_article_term`.`post_date` AS `post_date`,`v_article_term`.`post_content` AS `post_content`,`v_article_term`.`post_excerpt` AS `post_excerpt`,`v_article_term`.`post_title` AS `post_title`,`v_article_term`.`recommended` AS `recommended`,`v_article_term`.`smeta` AS `smeta`,`v_article_term`.`author` AS `author`,`v_article_term`.`author_name` AS `author_name` from `v_article_term` group by `v_article_term`.`id` ;

-- ----------------------------
-- View structure for v_article_term
-- ----------------------------
DROP VIEW IF EXISTS `v_article_term`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v_article_term` AS select `tab_posts`.`id` AS `id`,`tab_posts`.`post_author` AS `author`,`tab_posts`.`author_name` AS `author_name`,`tab_posts`.`post_keywords` AS `post_keywords`,`tab_posts`.`post_source` AS `post_source`,`tab_posts`.`post_date` AS `post_date`,`tab_posts`.`post_content` AS `post_content`,`tab_posts`.`post_excerpt` AS `post_excerpt`,`tab_posts`.`smeta` AS `smeta`,`tab_posts`.`post_title` AS `post_title`,`tab_posts`.`recommended` AS `recommended`,`tab_terms`.`name` AS `name` from ((`tab_terms` join `tab_posts`) join `tab_term_relationships`) where ((`tab_posts`.`id` = `tab_term_relationships`.`object_id`) and (`tab_terms`.`term_id` = `tab_term_relationships`.`term_id`)) ;
...
BEGIN
# 存储过程说明：对tab_posts中的author_name字段进行修改，为作者的名字,如果有已经修改过的记录，不修改。
# 注：游标必须放在所有变量定义之后
# 保存id对应的用户名
...
read_loop: LOOP
	...
	IF ABS((authorId  REGEXP '[^0-9]')-1) THEN
		...
		UPDATE v_article_term SET `author_name` = authorName where `author` = authorId;
	END IF;
END LOOP;
# 关闭游标
CLOSE cur;
...
IF is_exist THEN
	...
	post_source,post_date, post_content, post_excerpt, post_title,recommended,smeta,author,author_name 
...
ELSE
	...
	post_date, post_content, post_excerpt, post_title,recommended,smeta,author,author_name 
	FROM v_article_term GROUP BY id;
END IF;
END
;;
